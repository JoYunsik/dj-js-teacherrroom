import './Calendardate.css';

const CalendarDate = ({currDate,currYear,currMonth,currDay}) =>{
    const lastDateofMonth = new Date(currYear, currMonth + 1, 0).getDate() // getting last date of month
    const lastDateofLastMonth = new Date(currYear, currMonth, 0).getDate(); // getting last date of previous month
    const dates = []
    for(let i= currDate-currDay; i<currDate;i+=1){
        let renderdate = i>0 ? i : i+lastDateofLastMonth;
        if(renderdate===1){
            dates.push(<li key={renderdate} className="">{currMonth+1}/{renderdate}</li>);
        }
        else{
            dates.push(<li key={renderdate} className="">{renderdate}</li>);
        }        
    }
    for (let i = currDate; i<currDate+(7-currDay); i+=1){
        let isToday = i === new Date().getDate() && currMonth === new Date().getMonth() 
                        && currYear === new Date().getFullYear() ? "active" : "";
        
        let renderdate = i>lastDateofMonth ? i-lastDateofMonth : i;
        if(i===1&& currDate===1){
            dates.push(<li key={renderdate} className={isToday}>{currMonth+1}/{renderdate}</li>);
        }
        else if(isToday){
            dates.push(<li key={renderdate} className={isToday}>{currMonth+1}/{renderdate}</li>);
        }
        
        else if(renderdate===1){
            dates.push(<li key={renderdate} className={isToday}>{currMonth+2<=12? currMonth+2:1}/{renderdate}</li>);
        }
        else{
            dates.push(<li key={renderdate} className={isToday}>{renderdate}</li>);
        }
    }
    return(
        <ul className='days'> 
            {dates}
        </ul>
    )
}

export default CalendarDate;